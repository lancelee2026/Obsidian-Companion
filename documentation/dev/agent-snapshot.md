...
- `Implemented`: MIT `LICENSE` at the repo root for the Companion source. It does not license the paid activation key. Root `README.md` is the community-directory page and discloses the on-computer connection. Root `manifest.json` is the listing file community.obsidian.md reads. `versions.json` maps `0.1.9` to Obsidian `1.8.7`. `images/icon.png` is the 256px NoteFerry mark; the community Edit listing form uses a Lucide picker, not this PNG.
- `Implemented`: Public listing at `https://community.obsidian.md/plugins/noteferry-companion`. Automated review of `0.1.7` Completed. `0.1.8` Failed on APIs newer than 1.8.7. `0.1.9` removes those call sites. `fs` and vault-list disclosures stay: desktop seat file and note list.

...
- **Implemented:** loopback HTTP server, memory pairing store (hash-only secrets), FakeVault tests, Obsidian vault adapter, plugin shell with Allow/Deny pairing UI, esbuild `dist/main.js` + `manifest.json`.
